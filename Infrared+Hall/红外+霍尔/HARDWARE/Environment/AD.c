...
#define RL			5		// RL阻值
static float MQ2_R0; // 元件在洁净空气中的阻值
//static float MQ4_R0;
...
{ 
 ...
 GPIO_Init(GPIOA,&GPIO_Initstructre);
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_3, 4, ADC_SampleTime_55Cycles5);	//规则组序列3的位置，配置为通道3
	...
	ADC_InitStructure.ADC_NbrOfChannel = 4;										//通道数，扫描规则组的前通道
...
{   
	  u16 adcx;
	  adcx=Get_Adc_Average(0,100);//ADC1,取30次的平均值
...
{
	  u16 adcx;
	  adcx=Get_Adc_Average(1,100);//ADC1,取30次的平均值	
      float Vrl = 3.3f * adcx / 4096.f;//3.3v的参考电压，4096份
...
{
	  u16 adcx;
	  adcx=Get_Adc_Average(2,100);//ADC1,取30次的平均值	
    float Vrl = 3.3f * adcx / 4096.f;//3.3v的参考电压，4096份
...
{
	  u16 adcx;
	  adcx=Get_Adc_Average(2,100);//ADC1,取10次的平均值	
		return adcx;
}
